        ...
        <div class="container-fluid">
            ...
            <div class="row" id="dividers_container" >
                <div class="col-md-12" style="padding-top: 65px;">
                    ...
                    <div align="center"><h1><fmt:message key="restaurant.of"/> <c:out value="${utente.getNomeCognome()}"/></h1><br></div>

                    <c:forEach var="r" items="${listaMyRist}">
                        <div class="row jumbotron" style="border-radius: 10px; margin: 20px; padding-left: 20px; background-color: #ffffff">
                            <div class="col-md-12">
                                <div class="row">
                                    <h2>
                                        <a href="<c:out value="${pageContext.servletContext.contextPath}/ConfigurazioneRistorante?id_rist=${r.getId()}"/>"><c:out value="${r.getName()}"/></a>
                                    </h2>
                                </div>

                                <div class="row">
                                    <div class="col-md-4">
                                        <a href="<c:out value="${pageContext.servletContext.contextPath}/ConfigurazioneRistorante?id_rist=${r.getId()}"/>"><img src="<%= request.getContextPath()%><c:out value="${r.getFoto().get(0).getFotopath()}"/>" style="max-height: 100%; max-width: 100%"/></a>
                                    </div>
                                    <div class="col-md-4">
                                        <fmt:message key="users.vote"/> 
                                        <c:if test="${r.getVoto() == 0.0}">
                                            <fmt:message key="no.vote"/>
                                        </c:if>
                                        <c:if test="${r.getVoto() > 0}">
                                            <c:out value="${r.getVoto()}"/>
                                        </c:if><br><br>
                                        <fmt:message key="ranking"/>
                                        <c:if test="${r.getPosizioneClassifica() == -1}">
                                            <fmt:message key="no.vote"/>
                                        </c:if>
                                        <c:if test="${r.getPosizioneClassifica()>0}">
                                            <c:out value="${r.getPosizioneClassifica()}"/>
                                        </c:if><br>
                                        <fmt:message key="reviews"/> <c:out value="${r.getRecensioni().size()}"/> <br>
                                        <a href="http://www.google.com/maps/?q=<c:out value="${r.getLuogo().getAddress()}"/>"><fmt:message key="map"/></a>
                                    </div>
                                    <div class="col-md-4">
                                        <fmt:message key="cooking.type"/> <c:out value="${r.getCucina()}"/> <br>
                                        <fmt:message key="economy.zone"/> <c:out value="${r.getFascia()}"/> <br>
                                        <fmt:message key="web.site"/> <a href="<c:out value="${r.getLinksito()}"/>"><c:out value="${r.getName()}"/></a><br>           
                                    </div>
                                    <div class="col-md-4">
                                        <img src="<%= request.getContextPath()%><c:out value="${r.creaQR()}"/>" width="220" height="220"/>
                                    </div>
                                    <div class="col-md-4">
                                        <fmt:message key="actions.ristorante"/><br>
                                        <a href="<%= request.getContextPath()%>/privateRistoratore/ConfiguraOrariApertura?id_rist=<c:out value="${r.getId()}"/>"><fmt:message key="gestisci.orari"/></a> <br>
                                        <a href="<%= request.getContextPath()%>/private/LoadRistoranteServlet?id_rist=<c:out value="${r.getId()}"/>"><fmt:message key="modify.restaurant"/></a> <br>
                                        <a href="<%= request.getContextPath()%>/private/LoadRistoranteFotoServlet?id_rist=<c:out value="${r.getId()}"/>"><fmt:message key="add.foto"/></a>
                                   </div>
                                </div>
                            </div>
                        </div>
